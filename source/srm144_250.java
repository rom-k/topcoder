...
public class BinaryCode {
	public String[] decode (String message) {
		//int[] imes = new int[message.length()];
		int[] imes0 = new int[message.length()+1];
		int[] imes1 = new int[message.length()+1];
		imes0[0]=0;
		imes1[0]=1;
		for(int i = 0; i < message.length(); i++) {
			imes0[i+1] = Integer.parseInt(""+message.charAt(i));
			imes1[i+1] = Integer.parseInt(""+message.charAt(i));
		}
		//int[] imes1 = (int[])imes.close();
		//int[] imes2 = (int[])imes.close();
		for(int i = 0; i < message.length(); i++) {
			imes0[i+1] = imes0[i+1] - imes0[i];
			imes1[i+1] = imes1[i+1] - imes1[i];
		}
		/*
