...
sub calc_f {
    ...
    #TODO: InvertDistance or WalkingDistance
}
...
sub l_push {
    ...
    while ($head <= $tail) {
        my $next = int(($head + $tail) / 2);
        if ($s->[2] == $ary_ref->[$next]->[2]) {
            splice(@{$ary_ref}, $next, 0, ($s));
            return;
        }
        elsif ($s->[2] < $ary_ref->[$next]->[2]) {
            $tail = $next - 1;
        }
        else {
            $head = $next + 1;
