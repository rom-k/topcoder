...
cout <<"DEBUG: b" <<endl; //DEBUG
    
    // make goal charactor map
    map<char, int> g_map;
    FORI(i, 36) g_map[p_goal[i]] = i;
cout <<"DEBUG: c" <<endl; //DEBUG
    
    // calc f and sort open/close
    // O((Nop + Ncl))
//    for(set<pair<string, int> >::iterator it = open.begin(); it != open.end(); it++) {
//        if(open.insert(pair<string, int>(it->first, calc_f(it->first, p_goal, path[it->first], dist, g_map))).second) {
//            open.erase(it--);
//            continue;
//        }
//    }
//    for(set<pair<string, int> >::iterator it = close.begin(); it != close.end(); it++) {
//        if(close.insert(pair<string, int>(it->first, calc_f(it->first, p_goal, path[it->first], dist, g_map))).second) {
//            close.erase(it--);
//            continue;
//        }
//    }
...
cout <<"DEBUG: f" <<endl; //DEBUG
    n = 0;
    while(n < SZ(mp)) {
cout <<"DEBUG: f1" <<endl; //DEBUG
cout <<"DEBUG:    n        =" <<n <<endl; //DEBUG
cout <<"DEBUG:    SZ(mp)   =" <<SZ(mp) <<endl; //DEBUG
cout <<"DEBUG:    SZ(close)=" <<SZ(close) <<endl; //DEBUG
        for(set<pair<string, int> >::iterator it = close.begin(); it != close.end(); it++) {
            if (mp.find(it->first) != mp.end()) {
                close.erase(it--);
                n++;
...
//                        if(mp.find(it->first) != mp.end()) {
//                            if(/*open*/open_r.insert(pair<string, int>(it->first, calc_f(it->first, /*p_goal*/p_0, /*path*/path_r[it->first], /*dist*/dist_r, /*g_map*/g_map_r))).second) {
...
//                            }
//                            else {
//                                if(/*close*/close_r.insert(pair<string, int>(it->first, calc_f(it->first, /*p_goal*/p_0, /*path*/path_r[it->first], /*dist*/dist_r, /*g_map*/g_map_r))).second) {
...
cout <<"DEBUG: ------------------------------" <<endl; //DEBUG
        ...
        int t = mp[p_goal];
cout <<"DEBUG: it->first    =" <<it->first <<endl; //DEBUG
cout <<"DEBUG: it->second   =" <<it->second <<endl; //DEBUG
cout <<"DEBUG: mp[it->first]=" <<mp[it->first] <<endl; //DEBUG
