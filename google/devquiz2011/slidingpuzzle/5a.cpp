...
string a_star(string p_goal) {
    ...
    for(set<pair<string, int> >::iterator it = open.begin(); it != open.end(); it++) {
        cout <<"DEBUG: " <<it->first <<" " <<it->second <<" -> " <<calc_f(it->first, p_goal, path[it->first], dist, g_map) <<" " <<it->second <<endl;
...
//    // init (rev)
//    path_r[p_goal] = "";
//    open_r.insert(pair<string, int>(p_goal, calc_f(p_goal, p_goal, path_r[p_goal], dist_r, g_map_r)));
