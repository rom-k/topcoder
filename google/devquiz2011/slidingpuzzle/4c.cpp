...
string a_star(string p_goal) {
    ...
    // O((Nop + Ncl) * 36)
    cout <<"DEBUG: SZ(open)=" <<SZ(open) <<endl; //DEBUG
    set<string> st;
    for(set<pair<string, int> >::iterator it = open.begin(); it != open.end(); it++) {
        //it->second = calc_f(it->first, p_goal, path[it->first], dist, g_map);
        //st.insert(it->first);
        
        string p = it->first;
        int f = calc_f(p, p_goal, path[p], dist, g_map);
        
        cout <<"DEBUG: p=" <<p <<endl; //DEBUG
        cout <<"DEBUG: f=" <<it->second <<endl; //DEBUG
        cout <<"DEBUG: f_new=" <<f <<endl; //DEBUG
        
        if(it->second != f) {
            
            cout <<"DEBUG: add" <<endl; //DEBUG
            
            if(open.insert(pair<string, int>(p, f)).second) {
                cout <<"        => ok" <<endl;
            }
            else {
                cout <<"        => ng" <<endl;
                if(open.find(pair<string, int>(p, f)) != open.end()) {
                    cout <<"            (" <<open.find(pair<string, int>(p, f))->first <<endl;
                    cout <<"             " <<open.find(pair<string, int>(p, f))->second <<endl;
                }
            }
            
            //cout <<"DEBUG: remove" <<endl; //DEBUG
...
//    open.clear();
//    cout <<"DEBUG: SZ(open)=" <<SZ(open) <<endl; //DEBUG
//    cout <<"DEBUG: SZ(st)=" <<SZ(st) <<endl; //DEBUG
////    FORE(it, st, set<string>) {
//    for(set<string>::iterator it = st.begin(); it != st.end(); it++) {
//        //open.insert(pair<string, int>(*it, calc_f(*it, p_goal, path[*it], dist, g_map)));
//        string p = *it;
//        int f = calc_f(p, p_goal, path[p], dist, g_map);
//        if(DUMP) cout <<"DUMP: o " <<p <<" " <<path[p] <<" " <<f <<endl;
//        //open.insert(pair<string, int>(p, f));
//        if(open.insert(pair<string, int>(p, f)).second) {
//            cout <<"ok" <<endl;
//        }
//        else {
...
//        //if(DUMP) cout <<"DUMP: o " <<*it <<" " <<path[*it] <<" " <<calc_f(*it, p_goal, path[*it], dist, g_map) <<endl;
//        cout <<"DEBUG: SZ(open)=" <<SZ(open) <<" (++)" <<endl; //DEBUG
//    }
//    st.clear();
...
//        if (clear(*it, p_goal)) ret = path[*it];
//        //if(DUMP) cout <<"DUMP: c " <<*it <<" " <<path[*it] <<" " <<calc_f(*it, p_goal, path[*it], dist, g_map) <<endl;
//    }
